...
class GPT(nn.Module):
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        embedding = self.word_embeddings(context)
        positions = torch.arange(context.shape[1],device=context.device)
        embedding = embedding + self.poisiton_embedding(positions)
        output = self.layernorm1(self.transformer(embedding))
