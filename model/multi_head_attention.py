...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.multi_head = nn.ModuleList([self.SingleHeadAttention(embedding_dim, attention_dim // num_heads) for i in range(num_heads)])
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        results = []
        x = embedded 
        for i, l in enumerate(self.multi_head):
            results.append(l(x))
        contacted_data = torch.cat(results, dim=2)
        output = self.output_proj(contacted_data)
